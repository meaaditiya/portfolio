...
useEffect(() => {
  ...
  const fetchActiveProfileImage = async () => {
    try {
      const response = await fetch(`${process.env.backend_url}/api/profile-image/active`);
      if (response.ok) {
 ...
 const fetchPosts = async () => {
  try {
    setLoading(true);
    const response = await fetch(`${process.env.backend_url}/api/community/posts?page=${currentPage}&limit=10`);
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.backend_url}/api/community/posts/${postId}/like`, {
        method: 'POST',
  ...
  const fetchComments = async (postId) => {
    try {
      const response = await fetch(`${process.env.backend_url}/api/community/posts/${postId}/comments`);
      const data = await response.json();
  ...
  const handleComment = async (postId) => {
    ...
    try {
      const response = await fetch(`${process.env.backend_url}/api/community/posts/${postId}/comment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          comment: newComment,
  ...
  const handleCommentLike = async (commentId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.backend_url}/api/community/comments/${commentId}/like`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userEmail: userInfo.email, userName: userInfo.name })
      });
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      const response = await fetch(`${process.env.backend_url}/api/community/comments/${commentId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteReply = async (commentId, replyId) => {
    ...
    try {
      const response = await fetch(`${process.env.backend_url}/api/community/comments/${commentId}/replies/${replyId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userEmail: userInfo.email })
      });
  ...
  const handlePollVote = async (postId, optionIndex) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.backend_url}/api/community/posts/${postId}/vote`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          optionIndex,
  ...
  const submitQuiz = async (postId, totalQuestions) => {
    ...
    try {
      const response = await fetch(`${process.env.backend_url}/api/community/posts/${postId}/quiz-answer`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          answers,
...
const toggleMute = (postId) => {
  ...
  const handleQualityChange = (postId, quality) => {
    ...
    if (video) {
      ...
      video.src = `${process.env.backend_url}/api/community/posts/${postId}/media/video/${qualityPath}`;
    ...
    <div className="advanced-video-container">
      ...
      >
        <video
          ...
          src={`${process.env.backend_url}/api/community/posts/${post._id}/media/video/0`}
          ...
          {post.postType === 'image' && post.images && (
            <div className="post-images">
              {post.images.map((image, index) => (
                <img 
                  key={index}
                  src={`${process.env.backend_url}/api/community/posts/${post._id}/media/image/${index}`}
                  alt={`Post image ${index + 1}`}
                  className="post-image"
                  onClick={() => openFullScreenImage(`${process.env.backend_url}/api/community/posts/${post._id}/media/image/${index}`)}
                />
              ))}
            </div>
          )}
