...
const { liveCount, visitorStats, isConnected } = useVisitor();
  useEffect(() => {
    ...
    const fetchQuote = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_APP_BACKEND_URL}/api/quote`);
        if (response.ok) {
...
const fetchFirstBlog = async () => {
  try {
    const response = await fetch(`${import.meta.env.VITE_APP_BACKEND_URL}/api/blogs?status=published&limit=1`);
    if (response.ok) {
      const data = await response.json();
