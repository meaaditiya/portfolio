...
const { liveCount, visitorStats, isConnected } = useVisitor();
  useEffect(() => {
    ...
    const fetchQuote = async () => {
      try {
        const response = await fetch('https://aadibgmg.onrender.com/api/quote');
        if (response.ok) {
  ...
  try {
    const response = await fetch('https://aadibgmg.onrender.com/api/announcement/active');
    if (response.ok) {
      const data = await response.json();
      ...
      if (data.announcements && data.announcements.length > 0) {
        ...
        sortedAnnouncements.forEach(announcement => {
          if (announcement.hasImage) {
            imageMap[announcement._id] = `https://aadibgmg.onrender.com/api/announcement/${announcement._id}/image`;
...
const fetchFirstBlog = async () => {
  try {
    const response = await fetch('https://aadibgmg.onrender.com/api/blogs?status=published&limit=1');
    if (response.ok) {
      const data = await response.json();
...
{showAnnouncementOverlay && announcements.length > 0 && currentAnnouncement && (
 ...
 Pinged on:  {currentAnnouncement.createdAt 
  ...
  {currentAnnouncement.hasDocument && (
    <a 
      href={`https://aadibgmg.onrender.com/api/announcement/${currentAnnouncement._id}/document`}
      download
