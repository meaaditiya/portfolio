...
const copyTicketId = () => {
 ...
 let otpEndpoint = `${process.env.backend_url}/api/contact/send-otp`;
    if (isProject) {
      otpEndpoint = `${process.env.backend_url}/api/project/send-otp`;
    } else if (isAudio) {
      otpEndpoint = `${process.env.backend_url}/api/audio-contact/send-otp`;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isAudio) {
        ...
        const response = await axios.post(`${process.env.backend_url}/api/audio-contact`, formDataToSend, {
          headers: {
        ...
        const response = await axios.post(`${process.env.backend_url}/api/project/submit`, formDataToSend, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
...
const handleQuerySubmit = async (e) => {
  ...
  try {
    const response = await axios.post(`${process.env.backend_url}/api/queries/create`, queryFormData);
...
if (!/^QRY\d{12}$/.test(normalized)) {
  ...
  try {
    const response = await axios.get(`${process.env.backend_url}/api/queries/check/${ticketId}`);
