...
const [isVectorLoading, setIsVectorLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_APP_BACKEND_URL}/api/blogs?status=published&limit=${limit}`);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_BACKEND_URL}/api/blogs?status=published&limit=${newLimit}`);
