...
const [isVectorLoading, setIsVectorLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        const response = await fetch(`https://aadibgmg.onrender.com/api/blogs?status=published&limit=${limit}`);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const response = await fetch(`https://aadibgmg.onrender.com/api/blogs?status=published&limit=${newLimit}`);
